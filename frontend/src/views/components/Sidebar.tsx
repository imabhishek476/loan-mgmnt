...
const Sidebar: React.FC<SidebarProps> = observer(({ open, setOpen }) => {
  ...
  return (
    <aside
      className={`fixed md:static top-0 left-0 h-full flex flex-col justify-between border-r bg-white z-50 transform transition-all duration-300 ${
        open ? "w-64" : "w-16"
      }`}
    >
      {/* Logo Section */}
      <div
        className={`border-b border-gray-300 p-4 relative ${
          open ? "flex flex-row items-center justify-between" : "flex flex-col items-center"
        }`}
      >
        ...
        <button
          ...
          className={`p-2 rounded-md hover:bg-gray-200 bg-white transition-colors ${
            open ? "" : "mb-2"
          } lg:hidden`}
        >
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={`/${item.path}`}
              className={`flex items-center px-4 py-2 gap-3 rounded-lg font-medium transition-colors ${
                isActive ? "bg-green-800 text-white shadow-lg" : "text-gray-600 hover:bg-gray-100"
              }`}
            >
              {item.icon}
        ...
        {open ? (
          <div className="flex items-center justify-between gap-3 px-2 py-3">
            ...
            <button
              ...
              className="p-1.5 rounded-full text-white bg-red-600 hover:bg-red-700 hover:text-white transition-colors"
              title="Sign Out"
            >
